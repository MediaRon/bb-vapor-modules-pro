...
<div class="fl-bbvm-calendly-for-beaverbuilder">
	...
	if ( 'inline' === $settings->calendar_type ) :
		...
		<?php
	elseif ( 'popuptext' === $settings->calendar_type ) :
		$url = sprintf( 'https://calendly.com/%s', esc_attr( $settings->calendar_name ) );
		if ( 'yes' === $settings->hide_page_details ) {
			$url = add_query_arg(
				array(
		...
		<a href="" onclick="Calendly.initPopupWidget({url: '<?php echo esc_url( $url ); ?>'});return false;"><?php echo esc_html( $settings->popup_button_text ); ?></a>
