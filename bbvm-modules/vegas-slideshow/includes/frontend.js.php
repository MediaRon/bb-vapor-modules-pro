...
if( typeof jQuery.vegas !== 'undefined' ) {
	...
	jQuery(".fl-node-<?php echo $id; ?> .fl-bbvm-vegas-slideshow-for-beaverbuilder .slideshow-item").css("z-index", "15");
	jQuery(".fl-node-<?php echo $id; ?> .fl-bbvm-vegas-slideshow-for-beaverbuilder .slideshow-item").css("position", "relative").append("<div class=\"vegas\" style=\"position: absolute; width: 100%; height: 100%; z-index: 1; top: 0; left: 0;\"></div>");
	...
	$bullet_html .= '</div>';
	?>
	jQuery(".fl-node-<?php echo $id; ?> .fl-bbvm-vegas-slideshow-for-beaverbuilder").append('<?php echo $bullet_html; ?>');
	<?php endif; ?>
	<?php endif; ?>
	<?php if( 'yes' === $settings->show_arrows && count( $settings->images ) !== 1 ): ?>
		jQuery(".fl-node-<?php echo $id; ?> .fl-bbvm-vegas-slideshow-for-beaverbuilder .slideshow-item").after("<div class=\"arrow-left\"><i class=\"fas fa-arrow-left\"></i></div><div class=\"arrow-right\"><i class=\"fas fa-arrow-right\"></i></div>");
	<?php endif; ?>
	var meta_html = '';
	var meta_wrapper = '';
	...
	if ( '' !== meta_html ) {
		...
		jQuery(".fl-node-<?php echo $id; ?> .fl-bbvm-vegas-slideshow-for-beaverbuilder .slideshow-item").append(meta_html);
	}

	vegas_node_<?php echo $id; ?> = jQuery(".fl-node-<?php echo $id; ?> .fl-bbvm-vegas-slideshow-for-beaverbuilder .slideshow-item .vegas").vegas({
	...
	walk: function (index, slideSettings) {
		...
		jQuery('.fl-node-<?php echo $id; ?> .caption').html(slideSettings.caption);
		jQuery('.fl-node-<?php echo $id; ?> .sub-caption').html(slideSettings.subcaption);
		jQuery('.fl-node-<?php echo $id; ?> .link').html(anchor);
		jQuery('.fl-node-<?php echo $id; ?> .bullet').removeClass('active');
		jQuery('.fl-node-<?php echo $id; ?> .bullet_id_' + slideSettings.current_slide).addClass('active');
	}
	} );
	// End Vegas
	jQuery('.fl-node-<?php echo $id; ?> .arrow-left').click( function (e ) {
		vegas_node_<?php echo $id; ?>.vegas('previous');
	});
	jQuery('.fl-node-<?php echo $id; ?> .bullets .bullet').click( function (e ) {
		var slide = jQuery(this).data('id');
		vegas_node_<?php echo $id; ?>.vegas('jump', slide);
	});
	jQuery('.fl-node-<?php echo $id; ?> .arrow-right').click( function (e ) {
