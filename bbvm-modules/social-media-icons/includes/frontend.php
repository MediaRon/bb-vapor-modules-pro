...
if ( isset( $settings->social_form ) ) {
	...
	if ( 'color' === $settings->background_select ) {
		...
		$background_styles[] = 'background-image: ' . FLBuilderColor::gradient( $settings->background_gradient) . ';'; // phpcs:ignore
	}
	$background_gradient = '';
	echo '<div class="bbvm-module-social-wrapper" style="' . esc_attr( implode( ' ', $background_styles ) ) . '">';
	...
	foreach ( $settings->social_form as $index => $params ) {
		...
		foreach ( $maybe_icons as $attr => $value ) {
			if ( false !== strpos( $url, $attr ) ) {
				printf( '<li class="bbvm-module-social-icon-%s"><a href="%s" target="_blank" rel="noopener">%s</a></li>', esc_attr( $icon_size ), esc_url( $url ), bbvapor_modules_beaver_builder_social_get_svg( array( 'icon' => $value ), $settings ) ); // phpcs:ignore
			}
		}
	}
	echo '</ul>';
